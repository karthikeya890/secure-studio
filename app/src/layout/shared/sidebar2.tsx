    ...
    const renderSection = (data: any) => {
        return (
            <NavLink key={data.path} to={data.path} >
                {({ isActive }) => (
                    <Flex alignItems={"center"} gap={3} position={"relative"} m={3} my={2} p={2} opacity={isActive ? 1 : 0.8} style={{ marginRight: isActive ? "0px" : "auto" }} borderRadius={25} borderRightRadius={0} bgGradient={isActive ? "to-r" : ""} gradientFrom="blue.400" gradientTo="blue.200" >
                        {isActive && <>
                        ...
                        <Text color={"blackAlpha.800"} fontSize={"md"} fontWeight="bold">
                            {data.name}
                        </Text>
                    </Flex>
                )}
